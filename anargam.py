...
def get_anagrams(word, words_list):
    ...
    try:
        ...
        return [each for each in words_list if
                is_anagram(each.replace(' ', '').lower(), l_word)]
...
class TestAnagrams(unittest.TestCase):
    def test_positive_with_tuple(self):
        self.assertEqual(get_anagrams('dam', ('dam', 'ma d', 'mam')),
                         ['dam', 'ma d'])

    def test_positive_with_list(self):
        self.assertEqual(get_anagrams('parties',
                                      ['pastier', 'pirates', 'traipse',
                                       'piratis']),
                         ['pastier', 'pirates', 'traipse'])

    def test_positive_with_space(self):
        self.assertEqual(get_anagrams('Edward Gorey',
                                      ['Ogdred Weary', 'Regera Dowdy',
                                       'E G Deadworry']),
